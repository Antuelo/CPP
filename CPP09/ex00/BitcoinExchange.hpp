...
{
	private:
		std::map<std::string, float> _data; // le string la date, float le prix

		void 		load_database();
		bool		Date_valid(const std::string &date)					const; //pour valider la date
		bool		Value_valid(float value)							const; //pour valider entre 0 et 1000
		float		lookingfor_exchangerate(const std::string &date)	const; 
